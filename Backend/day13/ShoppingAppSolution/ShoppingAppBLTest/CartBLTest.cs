...
namespace ShoppingAppTest
{
    public class CartBLTest
    {
        ...
        public void Setup()
        {
            ...
            _cartBL = new CartBL(_repository, _services,_customerServices);
        }
        ...
        {
            // Arrange
            CartItem cartItem = new CartItem { CartId = 1, ProductId = 1, Quantity = 2, Price = 10.99, Discount = 1.50, PriceExpiryDate = DateTime.Now.AddDays(7) };
            ...
            // Act
            var result = _cartBL.AddItemToCart(cartItem,101);
        ...
        public void AddItemToCartMaxQuantityExceededException()
        {
            ...
            // Act & Assert
            Assert.Throws<MaxQuantityExceededException>(() => _cartBL.AddItemToCart(cartItem, 101));
        }
        ...
        {
            // Arrange
            CartItem cartItem1 = new CartItem { CartId = 1, ProductId = 1, Quantity = 2, Price = 10.99, Discount = 1.50, PriceExpiryDate = DateTime.Now.AddDays(7) };
            CartItem cartItem2 = new CartItem { CartId = 1, ProductId = 1, Quantity = 1, Price = 5.99, Discount = 0.75, PriceExpiryDate = DateTime.Now.AddDays(5) };
            Cart cart = new Cart { Id = 1, CustomerId = 101, Customer = new Customer(), CartItems = new List<CartItem>() };
            Product product = new Product { Id = 1, Price = 10.0, Name = "Product 1",
                QuantityInHand = 10 };
            _services.AddProduct(product);
            _cartBL.AddCart(cart);
            _cartBL.AddItemToCart(cartItem1, 101);

            // Act & Assert
            Assert.Throws<DuplicateCartItemException>(() => _cartBL.AddItemToCart(cartItem2,101));
        }
        ...
        {
            // Arrange
            CartItem cartItem = new CartItem { ProductId = 1, Quantity = 2, Price = 10.99, Discount = 1.50, PriceExpiryDate = DateTime.Now.AddDays(7) };
            Customer customer= new Customer() { Name="Avish" , Id=1,Age=23,Phone="1234567899"};
            _customerServices.AddCustomer(customer);
            Product product = new Product { Id = 1, Price = 10.0, Name = "Product 1",
                QuantityInHand = 10 };
            _services.AddProduct(product);
            // Act
            Cart result = _cartBL.AddItemToCart(cartItem, 1);
            //& Assert
            Assert.IsNotNull(result);
        ...
        public void AddCartFail()
        {
            ...
            Assert.Throws<CartIdNotPresentException>(()=> _cartBL.AddCart(cart));
        }
